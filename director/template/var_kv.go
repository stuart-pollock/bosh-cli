...
func (a *VarKV) UnmarshalFlag(data string) error {
	...
	if len(pieces) != 2 {
		return errors.Errorf("Expected var '%s' to be in format 'name=value'", data)
	}
	...
	if len(pieces[1]) == 0 {
		return errors.Errorf("Expected var '%s' to specify non-empty value", data)
	}
	...
	if err != nil {
		return errors.WrapErrorf(err, "Deserializing variables '%s'", data)
	}
