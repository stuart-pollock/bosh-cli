...
func (a *VarsEnvArg) UnmarshalFlag(prefix string) error {
	if len(prefix) == 0 {
		return errors.Errorf("Expected environment variable prefix to be non-empty")
	}
	...
	for _, envVar := range envVars {
		...
		if len(pieces) != 2 {
			return errors.Error("Expected environment variable to be key-value pair")
		}
		...
		if err != nil {
			return errors.WrapErrorf(err, "Deserializing YAML from environment variable '%s'", pieces[0])
		}
